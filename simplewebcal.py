...
def main(page: ft.Page):
	...
	def evaluate_expression(expr: str) -> str:
		...
		Supports 'i' or 'j' for the imaginary unit, ^ for power, and common math funcs.
		"""
		expr = expr.strip()
		expr = expr.replace("^", "**")
		expr = expr.replace("π", "pi")
	...
	def main(page: ft.Page):
		...
		# Keyboard input field (hidden-looking but accepts typing)
		...
		history = []  # store tuples (expr, result)
		...
		def on_submit_input(e):
			...
			press("=")
			# store last answer into input field
			input_field.value = expression["value"]
			# persist history (insert newest at front)
			try:
		...
		def render_history():
			# Build history controls; highlight selected
			controls = [
				ft.Row([
		...
		def on_key(e: ft.KeyboardEvent):
			# Esc clears, Ctrl+H toggles history, Enter submits if input focused
			if e.key == "Escape":
				expression["value"] = ""
				update_display()
			if e.key.lower() == "h" and e.ctrl:
				toggle_history()
			# navigate history with arrows when visible
			if show_history["value"] and history:
				...
				if e.key == "Enter":
					...
					if 0 <= idx < len(history):
						...
						update_display()
					return
			# If input field focused, Enter submits
			if e.key == "Enter":
		...
		for row in btns:
			controls = []
			for b in row:
				# button container for rounded surface
	...
	if __name__ == "__main__":
		if not check_libmpv():
			...
			print("To run the desktop runtime, install mpv/libmpv (Ubuntu/Debian):")
			print("  sudo add-apt-repository universe && sudo apt update && sudo apt install mpv")
